...
As duas primeiras consultas apresentam os mesmos registros porque não existem pares DNO e SALARY duplicados na tabela. Portanto, o DISTINCT não elimina nenhuma linha. A terceira consulta também apresenta os mesmos registros, porém ordenados primeiro por DNO e depois por SALARY.
...
Sim, são querys equivalentes, por conta da lei distributiva: C1 AND (C2 OR C3) = (C1 AND C2) OR (C1 AND C3)
...
Sim, são querys equivalentes, pois C1 OR (C2 AND C3) = (C1 OR C2) AND (C1 OR C3), pela lei distributiva.
...
A primeira e a terceira query são equivalentes. Por NOT (C1 AND C2) = (NOT C1) OR (NOT C2). A segunda query não é equivalente, pois utiliza AND entre as duas condições negadas.
...
A primeira e a terceira query são equivalentes. Pois NOT (C1 OR C2) = (NOT C1) AND (NOT C2).A segunda query não é equivalente, pois utiliza OR entre as condições negadas.
