---
title: "Lista05_FernandaVincenzo"
...
As seguintes questões utilizam a tabela PRESERVE
```{sql connection=con}
...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES*1.2 AS ACRES_NOVO FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES/3.0 AS ACRES_NOVO FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, FEE, (FEE+50)/2.0 AS FEE_NOVO FROM PRESERVE
```
...
SELECT AVG(ACRES) AS MEDIA_ACRES,
      MAX(ACRES) AS MAXIMO_ACRES,
      MIN(ACRES) AS MINIMO_ACRES
FROM PRESERVE
WHERE STATE='AZ'
```
...
```{sql connection=con}
SELECT PNAME, COUNT(*) AS QUANTIDADE FROM PRESERVE GROUP BY PNAME HAVING COUNT(*) > 1
```
...
```{sql connection=con}
SELECT AVG(ACRES*0.02) AS MEDIA_TAXA FROM PRESERVE
WHERE STATE='AZ'
```
...
```{sql connection=con}
SELECT COUNT(*) AS FUNCIONARIOS_20 FROM EMPLOYEE
WHERE DNO=20
```
13. Quantos departamentos possuem funcionários?
```{sql connection=con}
SELECT COUNT(DISTINCT DNO) AS QUANT_DEPARTAMENTOS FROM EMPLOYEE
```
